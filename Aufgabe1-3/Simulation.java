 ...
 *
 * @invariant totalYears >= 1
 * @invariant maxX und maxY >= 0
 * @invariant weather, plantPopulations, nestingSites, possiblePlanEvents, possibleBeeEvent, eventRandom != null
 * @invariant DAILY_EVENT_PROBABILITY im Bereich [0.0, 1.0]
 *
 */
public class Simulation {
     ...
     *
     * @pre totalYears >= 1
     * @pre nestingSites, plantPopulations, weather, gaussianDistribution, possibleEvents, possibleBeeEvents != null
     * @pre maxX und maxY >= 0
     * @post alle internen Variablen werden gesetzt.
     */
     ...
     *
     * @post die Simulation wird für totalYears durchgeführt und der Zustand der Population am Ende der Simulation wird gespeichert.
     * @post Die Logs (dailyLog, yearlyLog) enthalten die aufgezeichneten Daten entsprechend den Parametern.
     */
    ...
    /**
     * Simuliert die 240 Tage der Vegetationsperiode
     ...
     * dann wird ein tägliches Update der Pflanzenpopulation gemacht.
     * @param dailyLogging Log für jeden Tag, der simuliert wird
     * @param currentYear Int vom aktuellen Jahr für Log
     *
     * @pre currentYear im Bereich [1, totalYears]
     * @post Die dailyLog wird mit Daten für 240 Tage gefüllt, falls dailyLogging true ist.
     * @post Die Bienen- und Pflanzenpopulationen wurden für 240 Tage aktualisiert.
     *
     * BAD: Diese Methode ist sehr umfangreich und beinhaltet zu viele Aufgaben, wie
     *      die tägliche Aktualisierung der Bienen, Pflanzen und des Wetters. Dadurch
     *      wird die Koppelung zwischen Simulation, BeePopulation und Plantpopulation
     *      erhöht und somit leidet die Kohäsion der Klasse dadurch. Es wäre sinnvoll
     *      die einzelnen Aufgaben in mehrere kleinere Methoden aufzuteilen.
     */
     ...
     *
     * @pre currentYear im Bereich [1, totalYears], day im Bereich [1, 240]
     * @post dailyLog wird um die Daten des aktuellen Tages erweitert.
     *
     * GOOD: Der Ablauf dieser Methode ist klar uns sequentiell, da zuerst dailyLog
     *       erstellt wird, dann wird die gesamte Bienenpopulation berechnet und
     *       anschließend wird die Pflanzenpopulation durchlaufen. Es werden somit
     *       keine komplizierten verschachtelten Aufrufe gemacht und globale Variablen
     ...
     *
     * @pre currentYear im Bereich [1, totalYears]
     * @post Die yearlyLog wurde um die Daten dieses Jahres erweitert.
     */
     ...
     *
     * @post Die Populationswerte aller Bienenpopulationen wurden mittels updateRestPhase() aktualisiert.
     * @post Die Wuchskraft aller IPlantPopulation-Objekte wurde mittels updateRestPhase() aktualisiert.
     */
     ...
     *
     * @post Die finalen Zustände der Bienen- und Pflanzenpopulationen werden ausgegeben.
     */
     ...
     *
     * @post Die Inhalte von dailyLog und yearlyLog werden ausgegeben.
     */
     ...
     *
     * @pre sites und plants sind nicht null
     * @post falls die Koordinaten der sites und der plants in den definierten Grenzen liegen (0 <= x <= maxX, 0 <= y <= maxY), terminiert die Methode,
     * sonst wird eine IllegalArgumentException geworfen.
     */
