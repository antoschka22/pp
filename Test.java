/*
Miriam Reumann hat die Klasse HoneyBeePopulation implementiert, um Honigbienen als Konkurrenten und Bestäuber hinzuzufügen. Um die sprunghaften 1%/3%
...
Antonio Molina Gradischnig hat die alle Interfaces implementiert, außer IWeather. Mit den implementieren der Interfaces musste ich auch
...
Simon Oberdörfer hat den Record Coordinates erstellt, um allen Pflanzen und Nistplätzen einen Standort zu geben.
...
public class Test {
    ...
    private static final String CSV_WEATHER_PATH = "Aufgabe1-3/wetterwien.csv";
    ...
    public static void main(String[] args) {
        ...
        // Erstelle die Event die auftreten könnten
        ...
        runSummary(possibleEvents, possibleBeeEven);

        // Debugging der Gruppen
        runDebugSimulation(0, possibleEvents, possibleBeeEven);
        runDebugSimulation(1, possibleEvents, possibleBeeEven);
        //runDebugSimulation(2, possibleEvents, possibleBeeEven);
     ...
     */
    private static void runSummary(List<IPopulationEvent> possibleEvents, IBeeEvent possibleBeeEven) {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            for (int j = 0; j < SIMULATION_RUNS; j++) {
                ...
                GaussianDistribution dist = new GaussianDistribution(random);

                //Erstelle Nistplätze
                List<NestingSite> nests = createSites(random);

                List<IPlantPopulation> currentGroup;
                if (i == 0) {
                    currentGroup = createPlantsGroup1(random);
                } else if (i == 1) {
                    currentGroup = createPlantsGroup2(random);
                } else {
                    currentGroup = createPlantsGroup3(random);
                }

                IWeather weather;
                if (USE_CSV_WEATHER) {
                    weather = new CsvWeather(CSV_WEATHER_PATH, dist);
                } else {
                    weather = new RandomWeather(dist);
                }

                Simulation sim = new Simulation(YEARS, nests, currentGroup, weather,dist, FIELD_MAX_X, FIELD_MAX_Y, possibleEvents, possibleBeeEven);
                sim.run(false, false);
     ...
     */
    private static void runDebugSimulation(int i, List<IPopulationEvent> possibleEvents, IBeeEvent possibleBeeEven) {
        System.out.println("\n\n\n----------------------------------------------");
     ...
     */
    private static List<IPlantPopulation> createPlantsGroup1(Random random) {
     ...
     */
    private static List<IPlantPopulation> createPlantsGroup2(Random random) {
        List<IPlantPopulation> plants = new ArrayList<>();
        try {
            GaussianDistribution plantDist = new GaussianDistribution(random);
     ...
     */
    private static List<IPlantPopulation> createPlantsGroup3(Random random) {
        List<IPlantPopulation> plants = new ArrayList<>();
        try {
            GaussianDistribution plantDist = new GaussianDistribution(random);
     ...
     */
    private static List<NestingSite> createSites(Random random) {
